...
public class TicketService : ITicketService
{
    ...
    public async Task<TicketStatsDto> GetTicketStatsAsync()
    {
        ...
        var urgentCount = await _uow.Tickets.Query().CountAsync(t => t.Priority == "URGENT" && t.Status != "RESOLVED" && t.Status != "CLOSED");
        var highCount = await _uow.Tickets.Query().CountAsync(t => t.Priority == "HIGH" && t.Status != "RESOLVED" && t.Status != "CLOSED");

        var byAssignee = await _uow.Tickets.Query()
            .Include(t => t.AssignedToUser)
