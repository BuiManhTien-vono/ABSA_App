...
using HigenAbsa.Domain.Entities;
using HigenAbsa.Application.Common;
using HigenAbsa.Application.DTOs.Store;
...
namespace HigenAbsa.Application.Services.Store;
...
public class StoreService : IStoreService
{
    ...
    public StoreService(IUnitOfWork uow, IInferenceService inferenceService)
    {
        _uow = uow;
    ...
    {
        return await _uow.Platforms.Query()
            .Where(p => p.IsActive)
            .OrderBy(p => p.Id)
    ...
    {
        var query = _uow.StoreConnections.Query()
    ...
    {
        return await _uow.StoreConnections.Query()
            .Include(s => s.Platform)
            .Where(s => s.Id == id)
            .Select(s => new StoreConnectionDto
            {
    ...
    public async Task<StoreConnectionDto> CreateStoreAsync(CreateStoreRequest request)
    {
        ...
        _uow.StoreConnections.AddAsync(store);
    ...
    {
        var store = await _uow.StoreConnections.GetByIdAsync(id);
    ...
    {
        var store = await _uow.StoreConnections.Query()
        ...
        if (store.StoreCodeOnPlatform.StartsWith("EXCEL_"))
        {
            ...
            var aiAnalyses = await _uow.ReviewAIAnalyses.Query().Where(a => reviewIds.Contains(a.ReviewId)).ToListAsync();
            var aspects = await _uow.ReviewAspects.Query().Where(a => reviewIds.Contains(a.ReviewId)).ToListAsync();

            _uow.ReviewAspects.RemoveRange(aspects);
            _uow.ReviewAIAnalyses.RemoveRange(aiAnalyses);
            _uow.Reviews.RemoveRange(store.Reviews);
            _uow.Products.RemoveRange(store.Products);
            _uow.StoreConnections.Remove(store);
        }
        else
        {
    ...
    {
        var platform = await _uow.Platforms.Query().FirstOrDefaultAsync(p => p.Code == "LAZADA")
            ?? await _uow.Platforms.Query().FirstAsync();

        const string mockStoreCode = "LAZADA_MOCK_OFFICIAL_VN";
        var existing = await _uow.StoreConnections.Query()
            .Include(s => s.Products)
            .Include(s => s.Reviews)
        ...
        foreach (var pData in mockProducts)
        {
            ...
            _uow.Products.AddAsync(product);
            ...
            foreach (var rData in pData.Reviews)
            {
                ...
                _uow.Customers.AddAsync(customer);
                ...
                try
                {
                    ...
                    _uow.ReviewAIAnalyses.AddAsync(aiAnalysis);

                    foreach (var asp in pred.AspectSentiments ?? [])
                    {
                        _uow.ReviewAspects.AddAsync(new ReviewAspect
                        {
                            ReviewId = review.Id,
                ...
                catch
                {
                    ...
                    _uow.ReviewAIAnalyses.AddAsync(new ReviewAIAnalysis
                    {
                        Id = Guid.NewGuid(),
                        ReviewId = review.Id,
