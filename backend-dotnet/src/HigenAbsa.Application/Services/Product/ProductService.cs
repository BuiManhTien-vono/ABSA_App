...
using HigenAbsa.Application.Common;
using HigenAbsa.Application.DTOs.Product;
using HigenAbsa.Application.DTOs.Review;

using HigenAbsa.Domain.Interfaces;
using HigenAbsa.Domain.Entities;

namespace HigenAbsa.Application.Services.Product;
...
public class ProductService : IProductService
{
        ...
        int page, int pageSize, Guid? storeId, string? search)
    {
        ...
        var pagedQuery = query
            ...
            .Select(p => new ProductDto
            {
                ...
                ReviewCount = _uow.Reviews.Query().Count(r => r.ProductId == p.Id),
                AverageRating = _uow.Reviews.Query().Where(r => r.ProductId == p.Id).Any()
                    ? _uow.Reviews.Query().Where(r => r.ProductId == p.Id).Average(r => (double)r.Rating) : 0
            });
    ...
    public async Task<ProductDetailDto?> GetProductByIdAsync(Guid id)
    {
        ...
        var reviews = _uow.Reviews.Query().Where(r => r.ProductId == id);
    ...
    public async Task<ProductSentimentSummaryDto?> GetProductSentimentSummaryAsync(Guid productId)
    {
        ...
        var reviews = _uow.Reviews.Query().Where(r => r.ProductId == productId);
        ...
        var aspectBreakdown = await _uow.ReviewAspects.Query()
            .Where(a => a.Review != null && a.Review.ProductId == productId)
