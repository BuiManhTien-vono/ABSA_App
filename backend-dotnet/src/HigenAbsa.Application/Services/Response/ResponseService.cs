...
using HigenAbsa.Domain.Entities;
using HigenAbsa.Application.Common;
using HigenAbsa.Application.DTOs.Response;
using HigenAbsa.Application.DTOs.Review;

using HigenAbsa.Domain.Interfaces;

namespace HigenAbsa.Application.Services.Response;
...
public class ResponseService : IResponseService
{
    ...
    {
        var query = _uow.ResponseTemplates.Query()
            .Include(t => t.Store)
    ...
    public async Task<ResponseTemplateDto> CreateTemplateAsync(CreateTemplateRequest request, Guid userId)
    {
        ...
        _uow.ResponseTemplates.AddAsync(template);
    ...
    {
        var template = await _uow.ResponseTemplates.Query()
            .Include(t => t.Store)
            .Include(t => t.CreatedByUser)
    ...
    {
        var template = await _uow.ResponseTemplates.GetByIdAsync(id);
        if (template == null) return false;

        _uow.ResponseTemplates.Remove(template);
        await _uow.SaveChangesAsync();
        return true;
    }
    ...
    {
        var query = _uow.AutomationRules.Query()
            .Include(r => r.Store)
    ...
    public async Task<AutomationRuleDto> CreateRuleAsync(CreateRuleRequest request)
    {
        ...
        _uow.AutomationRules.AddAsync(rule);
        await _uow.SaveChangesAsync();

        var store = await _uow.StoreConnections.GetByIdAsync(request.StoreId);
    ...
    {
        var rule = await _uow.AutomationRules.Query()
            .Include(r => r.Store)
            .Include(r => r.SelectedTemplate)
        ...
        Guid reviewId, SendResponseRequest request, Guid userId)
    {
        ...
        _uow.ReviewResponses.AddAsync(response);
    ...
    {
        return await _uow.ReviewResponses.Query()
